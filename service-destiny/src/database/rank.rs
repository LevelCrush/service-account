...
pub async fn pvp_based(display_name: &str, modes: &[i32], pool: &SqlitePool) -> Vec<LeaderboardEntryResult> {
    if modes.is_empty() {
        return Vec::new();
    }
...
pub async fn generic(display_name: &str, modes: &[i32], pool: &SqlitePool) -> Vec<LeaderboardEntryResult> {
    if modes.is_empty() {
        return Vec::new();
    }
