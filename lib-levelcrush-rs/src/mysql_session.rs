/*
adapted from: https://raw.githubusercontent.com/jbr/async-sqlx-session/main/src/mysql.rs
original repository: https://github.com/jbr/async-sqlx-session/tree/main

Changes:
* Adjusted logging method
* Using tokio instead of async-std
* Using same version of sqlx that this lib (levelcrush) uses
*/
use axum::async_trait;
use axum_sessions::async_session::{serde_json, Result};
...
impl MySqlSessionStore {
    ...
    pub async fn migrate(&self) -> sqlx::Result<()> {
        tracing::info!("migrating sessions on `{}`", self.table_name);
...
impl SessionStore for MySqlSessionStore {
    async fn load_session(&self, cookie_value: String) -> Result<Option<Session>> {
        ...
        let mut connection: PoolConnection<MySql> = self.connection().await?;
