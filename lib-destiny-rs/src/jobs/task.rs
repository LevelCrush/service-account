...
pub async fn profile(membership_id: i64, membership_type: i64, state: &AppState) -> anyhow::Result<Vec<i64>> {
    ...
    let profile_response = if membership_type <= 0 {
        ...
        let profile = crate::api::member::memberships_by_id(membership_id, &state.bungie).await?;
        if let Some(profile) = profile {
            ...
            crate::api::member::profile(target_membership, profile.membership_type as i64, &state.bungie).await?
        } else {
            None
        }
    } else {
        crate::api::member::profile(membership_id, membership_type, &state.bungie).await?
    };
...
pub async fn profile_search(bungie_name: &str, state: &AppState) -> anyhow::Result<Option<ProfileSearchResults>> {
    ...
    let api_result = crate::api::member::search(bungie_name, &state.bungie).await?;
...
pub async fn clan_roster(group_id: i64, state: &AppState) -> anyhow::Result<HashMap<i64, i64>> {
    ...
    let api_response = crate::api::clan::roster(group_id, &state.bungie).await?;
...
pub async fn clan_info(group_id: i64, state: &AppState) -> anyhow::Result<()> {
    ...
    let api_response = crate::api::clan::info(group_id, &state.bungie).await?;
    if let Some(api_response) = api_response {
        tracing::info!("Starting to sync for group: {}", group_id);
...
pub async fn clan_info_by_membership(
    ...
    let mut group = 0;
    let api_response = crate::api::clan::from_membership(membership_id, membership_type, &state.bungie).await?;
    if let Some(api_response) = api_response {
...
pub async fn instance_data(instance_ids: &[i64], state: &AppState) -> anyhow::Result<()> {
    for instance_id in instance_ids.iter() {
        ...
        let response = crate::api::instance::carnage_report(*instance_id, &state.bungie).await?;
