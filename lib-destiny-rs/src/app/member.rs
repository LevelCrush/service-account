...
pub async fn profile(membership_id: i64, state: &mut AppState) -> Option<MemberResult> {
    ...
    if call_api {
        let user_card = crate::api::member::memberships_by_id(membership_id, &state.bungie).await;
        if let Ok(Some(user_card)) = user_card {
            let profile_response =
                crate::api::member::profile(membership_id, user_card.membership_type as i64, &state.bungie).await;
...
pub async fn by_bungie_name(bungie_name: &str, state: &mut AppState) -> Option<MemberResult> {
    ...
    if call_api {
        ...
        if let Ok(Some(api_result)) = &api_result {
            ...
            let profile_response = crate::api::member::profile(membership_id, membership_type, &state.bungie).await;
