...
pub async fn get(group_id: GroupId, state: &mut AppState) -> Option<ClanInfoResult> {
    ...
    if call_api {
        ...
        let clan_info_api_future =
            tokio::spawn(async move { crate::api::clan::info(group_id, &arc_ci_state.bungie).await });
        let clan_roster_api_future =
            tokio::spawn(async move { crate::api::clan::roster(group_id, &arc_roster_state.bungie).await });
