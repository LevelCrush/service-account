            ...
            origin: (origin, callback) => {
                {
                    ...
                    const allow_all = config.origins.at(0) === '*';
                    const allow_origin = allow_all || config.origins.includes(origin);
                    callback(
                        allow_origin ? null : new Error('Domain did not pass CORS'),
                        allow_origin ? true : undefined,
                    );
                }
            },
        }),
